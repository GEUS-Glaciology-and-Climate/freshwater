...
For each of these two geospatial domains, there are two runoff databases: MAR and RACMO.
...
+ The =id= column may not be unique for multiple reasons:
  ...
  + =id='s are unique within a dataset (i.e. =land=, and =ice=), but note between datasets.
...
|-------+--------+---------+--------+---------+----------+------+--------+----------+----------+-----------+-----------+---------+---------|
|     0 | 112448 | -51.233 | 67.156 | -272150 | -2491850 |   42 | land   | False    |          |           |           |         |         |
...
Note that the =domain=, =k=, and =upstream= columns can be used to subset the table.
