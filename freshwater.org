...
ds["station"] = (("station"), df.columns.astype(str).astype(np.uint32))
...
if 'ice' in BSVFILE:
    ds["coast_id"] = (("station"), np.uint32(meta.loc["coast_id"].astype(np.float32).replace(np.nan, value=-1)))
