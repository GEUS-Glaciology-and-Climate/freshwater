...
doi:10.22008/promice/freshwater citep:GEUS_freshwater_paper and code is available at http://github.com/GEUS-PROMICE/freshwater citep:github_freshwater.
#+END_abstract
...
where =discharge.py= is the provided script, =./freshwater= is the folder containing the downloaded data, and =--discharge= tells the program to return RCM discharge (as opposed to =--outlets= which would return basin and outlet information). The program documentation and usage examples are available at http://github.com/GEUS-PROMICE/freshwater citep:github_freshwater.
...
The code and a website for postpublication updates are available at https://github.com/GEUS-PROMICE/freshwater citep:github_freshwater where we document changes to this work and use the GitHub Issues feature to collect suggested improvements, document those improvements as they are implemented, and document problems that made it through review. This version of the document is generated with git commit version \input{|"git describe --always --dirty='*'"}.
...
ds["M2019_basin"] = (("station"), meta.loc["M2019_basin"].replace(np.nan, value=-1).astype(np.uint32))
ds["M2019_basin"].attrs["long_name"] = "Mouginot (2019) basin nearest outlet"
...
ds["M2020_gate_dist"].attrs["long_name"] = "Distance from Mankoff (2020) gate to nearest outlet"
...
wget https://dataverse01.geus.dk/api/access/datafile/:persistentId?persistentId=doi:10.22008/promice/data/ice_discharge/gates/v02/GSEWLR -O ./tmp/Mankoff_2020_gates/gates.pgk

md5sum ./tmp/Mankoff_2020_gates/*
# 437249095c2c9c0bd362dcb2860eb763  ./tmp/Mankoff_2020_gates/gates.pgk
